...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device,  nepochs=100,optimizer_type="adam", lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 momentum=0, wd=0,  logger=None, exemplars_dataset=None, lamb=1, alpha=0.5, damping=0.1, fim_sampling_type='max_pred',
                 fim_num_samples=-1):
    ...
    def compute_fisher_matrix_diag(self, trn_loader):
        ...
        for x , targets, _  in tqdm(itertools.islice(trn_loader, n_samples_batches)):
    ...
    def post_train_process(self, t, trn_loader, trn_loader_crops):
        ...
        curr_fisher = self.compute_fisher_matrix_diag(trn_loader_crops)
